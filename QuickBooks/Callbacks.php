...
class QuickBooks_Callbacks
{
    ...
    public static function callAuthenticate($Driver, $callback, $username, $password, &$customauth_company_file, &$customauth_wait_before_next_update, &$customauth_min_run_every_n_seconds)
    {
        ...
        if ($type == QuickBooks_Callbacks::TYPE_OBJECT_METHOD) // Object instance method hook
        {
            ...
            if ($type == QuickBooks_Callbacks::TYPE_FUNCTION) // Function hook
            {
                $err = '';
                ...
                if ($type == QuickBooks_Callbacks::TYPE_STATIC_METHOD) // Static method hook
                {
                    if ($Driver) {
    ...
    public static function callSAMLCallback($Driver, $callback, $auth_id, $ticket, $target_url, $realm_id_pseudonym, $config, &$err)
    {
        ...
        if ($type == self::TYPE_OBJECT_METHOD) // Object instance method hook
        {
            ...
            if ($type == self::TYPE_FUNCTION) // Function hook
            {
                $err = '';
                ...
                if ($type == self::TYPE_STATIC_METHOD) // Static method hook
                {
                    if ($Driver) {
            ...
            if (is_array($callback)) {
                ...
                if (is_string($callback) and false === strpos($callback, '::')) {
                    ...
                    if (is_string($callback) and false !== strpos($callback, '::')) {
                        ...
                        if (is_object($callback) and $callback instanceof QuickBooks_Hook) {
                ...
                if ($type == self::TYPE_OBJECT_METHOD) // Object instance method hook
                {
                    ...
                    if ($type == self::TYPE_FUNCTION) // Function hook
                    {
                        ...
                        if ($type == self::TYPE_STATIC_METHOD) // Static method hook
                        {
                            ...
                            if ($type == self::TYPE_HOOK_INSTANCE) {
    ...
    public static function callErrorHandler($Driver, $errmap, $errnum, $errmsg, $user, $requestID, $action, $ident, $extra, &$errerr, $xml, $callback_config)
    {
        ...
        if (isset($errmap[$errnum])) {
            ...
            if (isset($errmap[$action])) {
                ...
                if (isset($errmap['!'])) // catch-all error handler		//  and function_exists($this->_onerror['!'])
                {
                    $callback = $errmap['!'];
                } else {
                    if (isset($errmap['*'])) // catch-all error handler		//  and function_exists($this->_onerror['*'])
